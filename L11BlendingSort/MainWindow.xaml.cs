...
namespace L11BlendingSort
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private Dictionary<float, int> windowsSort = new Dictionary<float, int>();
        private List<Vector3> windows = new List<Vector3>()
        {
        ...
        List<float> sortIndex = new List<float>();
        ...
        private void Glc_Load(object sender, EventArgs e)
        {
            ...
            grassTexture = Texture.LoadFromFile("blending_transparent_window.png", TextureWrapMode.ClampToEdge, TextureWrapMode.ClampToEdge, TextureUnit.Texture2);
            ...
            {
                windowsSort.Add((_camera.Position - windows[i]).Length, i);
                sortIndex.Add((_camera.Position - windows[i]).Length);
            }
        ...
        private void Glc_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
        {
            ...
            {
                model = Matrix4.Identity;
                model *= Matrix4.CreateTranslation(windows[windowsSort[sortIndex[i]]]);
                shader.SetMatrix4("model", model);
                GL.DrawArrays(BeginMode.Triangles, 0, 6);
            }
